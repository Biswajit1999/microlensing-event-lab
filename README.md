...
3. **`data/reference.json`** ships a small, fixed set of published-morphology anchor points (baseline / rising wing / peak / falling wing / baseline-return) drawn as fixed markers on the plot next to the live model curve, so a user can visually sanity-check the simulated curve's shape against a canonical OGLE-style event.
